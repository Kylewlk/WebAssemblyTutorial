...
## 调试技巧
* 如果wasm文件太大，Debug版本浏览器加载时会报错，可以使用 `-O1` 优化 `target_compile_options(myTarget PRIVATE -O1)`, `target_link_options(myTarget PRIVATE -O1)`  [Optimizing Code](https://emscripten.org/docs/optimizing/Optimizing-Code.html)，`-O1`优化会导致一些局部变量看不了，可在函数上加上 __attribute__((noinline, optnone))。不要用O2或O3优化，O2、O3会对可执行文件两次扫描，大幅优化导致大部分局部变量看不到值，而且会增加编译时间。
